...
def is_county_marker(line):
    """Check if line is a county name marker - MUST be standalone county name only"""
    counties = [
    ...
    # MUST be ONLY the county name, nothing else
    # NOT if it has other content like office locations
    for county in counties:
        # Check if line is EXACTLY "CountyName County" or EXACTLY "CountyName"
        if re.match(rf'^{re.escape(county)}\s+County$', line_clean, re.IGNORECASE):
            return county
        if line_clean == county:
            return county
    return None
...
def process_extracted_data(rows, default_county):
    """Process extracted rows into structured data - NOW TRACKS COUNTY CHANGES"""
    processed_rows = []
    current_county = default_county
    
    for row in rows:
        if len(row) < 1:
            continue
        
        # Check if FIRST element (and ONLY first element) is a county marker
        # Only check first element to avoid false positives
        if len(row) == 1:
            county_check = is_county_marker(row[0])
            if county_check:
                current_county = county_check
                logger.info(f"  County changed to: {current_county}")
                continue
        
        # Need at least 3 parts for a data row
        if len(row) < 3:
            continue
...
def extract_table_from_page(page):
    ...
    try:
        # Get page text for initial county detection
        page_text = page.extract_text() or ""
        
        # Find first county on page as default
        counties = [
            "Mombasa", "Kwale", "Kilifi", "Tana River", "Lamu", "Taita Taveta", "Garissa",
            "Wajir", "Mandera", "Marsabit", "Isiolo", "Meru", "Tharaka-Nithi", "Embu",
            "Kitui", "Machakos", "Makueni", "Nyandarua", "Nyeri", "Kirinyaga", "Murang'a",
        ...
        for county in counties:
            if county.upper() in text_upper:
                default_county = county
                break
        
        logger.info(f"Processing page starting with {default_county} county")
        
        # Extract table data
        raw_rows = extract_table_data(page)
        
        # Process the extracted data - now tracks county changes within page
        processed_rows = process_extracted_data(raw_rows, default_county)
...
def main():
    ...
    try:
        ...
        county_counts = {}
        for row in unique_rows:
            county_counts[row['county']] = county_counts.get(row['county'], 0) + 1
        
        logger.info("\nConstituencies per county:")
        for county in sorted(county_counts.keys()):
            logger.info(f"  {county}: {county_counts[county]}")
