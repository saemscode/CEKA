...
def analyze_diff(last_hash, current_hash, commit_msg):
    try:
        ...
        # If last_hash is empty, default to a minor bump
        if not last_hash:
            return 'minor'
...
def update_files(new_version, current_hash_short):
    ...
    # Update About.tsx (Hardcoded string replacement)
    if os.path.exists(ABOUT_TSX):
        with open(ABOUT_TSX, 'r', encoding='utf-8') as f:
            content = f.read()
        
        # Replace the CEKA V... pattern (Case insensitive)
        new_content = re.sub(r'CEKA V\d+\.\d+\.\d+', f'CEKA V{new_version}', content, flags=re.IGNORECASE)
        if new_content != content:
            with open(ABOUT_TSX, 'w', encoding='utf-8') as f:
                f.write(new_content)
...
def main():
    ...
    if not current_hash or current_hash_short == last_hash:
        ...
        # We output the current version anyway so actions can use it
        print(f"::set-output name=version::{state['version']}")
        print(f"::set-output name=bumped::false")
        sys.exit(0)
    ...
    # GitHub Actions integration - pass the outputs out
    print(f"::set-output name=version::{new_version}")
    print(f"::set-output name=bumped::true")
    print(f"::set-output name=tag_name::v{new_version}")
