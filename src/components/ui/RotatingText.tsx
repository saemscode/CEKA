...
const RotatingText = forwardRef<any, RotatingTextProps>((props, ref) => {
  ...
  useEffect(() => {
    ...
    const timerId = setTimeout(() => next(), currentDuration);
  ...
  return (
    <div
      ...
      style={{ height: fixedHeight, display: "inline-flex", position: "relative" }}
    >
      <div
        ref={measureRef}
        style={{
          ...
          pointerEvents: "none",
          height: fixedHeight,
          lineHeight: fixedHeight as string,
        }}
      />
      <motion.div
        className={cn("text-rotate-wrapper")}
        animate={{ width }}
        transition={{ type: "spring", damping: 30, stiffness: 100, mass: 0.5, restDelta: 0.001 }}
        style={{ height: fixedHeight, minWidth: 0, overflow: "hidden", display: "inline-flex", justifyContent: "center" }}
      >
        <motion.span
          className={cn("text-rotate", mainClassName)}
          {...rest}
          layout
          style={{ height: fixedHeight, lineHeight: fixedHeight as string }}
        >
