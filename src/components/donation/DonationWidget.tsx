...
const DonationWidget = ({ onTimedOut }: { onTimedOut?: () => void }) => {
  ...
  // Define fixed position for the widget wrapper
  const widgetFixedPosition = {
    bottom: isMobile ? "80px" : "20px",
    right: "20px"
  };
  
  // Animation variants - now relative to the fixed wrapper
  const containerVariants = {
    ...
    visible: (expanded) => ({ 
      ...
      scale: 1,
      x: expanded ? "calc(50vw - 50% - 20px)" : 0,
      y: expanded ? "calc(-50vh + 50% + (80px / 2))" : 0,
      transition: {
        type: "spring",
        stiffness: 380,
        damping: 30,
